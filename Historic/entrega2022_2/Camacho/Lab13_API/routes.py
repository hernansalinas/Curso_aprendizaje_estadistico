...
@app.route("/numerospares", methods=["GET", "POST"])
def numerospares(): 
    
    num = request.args.get("numerospares", "")
    num = int(num)
    if num:
        even = [j for j in range(num) if j % 2 == 0]
    else:
        even = ""

    return render_template("numerospares.html", value=even)
